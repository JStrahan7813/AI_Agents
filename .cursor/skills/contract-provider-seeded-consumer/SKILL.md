...
- Helps provider teams derive initial **consumer-side Pact contracts** by scanning existing provider API repositories.  
- Works in provider-side **C# (.NET)** and **JavaScript/TypeScript** services to discover endpoints, co-design a scenario matrix, and shape consumer behaviours.  
- Generates **only consumer-side Pact tests** (and an optional lightweight client wrapper) after completing an interactive questionnaire; it never creates provider verification tests.  

## Usage

Use this skill from within a **provider service repository** (the API implementation), not a consumer repo.  
Be prepared to go through the **Interactive Questionnaire (Phases 1–4)** before any Pact tests or client code are generated.

**Example prompts**

- "I'm in an ASP.NET Core provider repo with no consumer yet. Help me design and generate provider-seeded **consumer-side Pact tests** for a few core endpoints."  
- "Scan this Node/Express API and guide me through your questionnaire to design scenarios, then generate **TypeScript Pact consumer tests** from this provider."  
- "From this provider repository, help me pick 3–5 key endpoints and, after your questionnaire, produce initial **consumer Pact contracts** plus C# xUnit consumer tests."
...
- **Runtime / SDK sanity check (for .NET providers)**  
   ...
   - Inspect the main `.csproj` and detect `TargetFramework` (for example `net8.0`, `net10.0`).  
...
Then:
- Scan those paths for common routing patterns:
...
11. **Client Wrapper**  
    ...
    - If a wrapper exists, inspect it and infer:
